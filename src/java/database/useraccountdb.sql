DROP SCHEMA IF EXISTS `useraccountdb`;
CREATE SCHEMA IF NOT EXISTS `useraccountdb` DEFAULT CHARACTER SET latin1;
USE `useraccountdb`;

CREATE TABLE IF NOT EXISTS `useraccountdb`.`category` (
  `category_id` INT(11) NOT NULL AUTO_INCREMENT,
  `category_name` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`category_id`));
CREATE TABLE IF NOT EXISTS `useraccountdb`.`role` (
  `role_id` INT(11) NOT NULL,
  `role_name` VARCHAR(25) NOT NULL,
  PRIMARY KEY (`role_id`));

CREATE TABLE IF NOT EXISTS `useraccountdb`.`user` (
  ...
  CONSTRAINT `fk_user_role`
    FOREIGN KEY (`role`)
    REFERENCES `useraccountdb`.`role` (`role_id`));

CREATE TABLE IF NOT EXISTS `useraccountdb`.`item` (
  ...
  CONSTRAINT `fk_items_categories`
    ...
    REFERENCES `useraccountdb`.`category` (`category_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_items_owner`
    FOREIGN KEY (`owner`)
    REFERENCES `useraccountdb`.`user` (`email`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
